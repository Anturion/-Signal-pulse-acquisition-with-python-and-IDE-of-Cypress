...
fs = 250
order, lowpass = filter_design(fs, locutoff = 0, hicutoff =7, revfilt = 0); #Se define el filtro pasabajas
#plot
mfreqz(lowpass,1,order, fs/2);

order, highpass = filter_design(fs, locutoff = 1, hicutoff = 0, revfilt = 1); #Se define el filtro pasa altas
#plot
mfreqz(highpass,1,order, fs/2);
...
Pulso=pd.read_csv('Datos_con_ambos_filtros.csv')
Pulso=Pulso['Datos']
f,pot= signal.welch(Pulso,fs, 'hanning', fs*2,fs)

plt.plot(Pulso)
plt.title('Señal obteniada con filtros analógicos')
plt.grid()
plt.show()
plt.plot(f,pot)
plt.title('Potencia de la señal con filtros analógicos')
plt.xlabel('Potencia')
...
Pulso=pd.read_csv('Datos_puros.csv')
Pulso=Pulso['Datos']
Pulso_filtrado=signal.filtfilt(lowpass, 1, Pulso)
f,pot= signal.welch(Pulso,fs, 'hanning', fs*2,fs)
f2,pot2=signal.welch(Pulso_filtrado,fs, 'hanning', fs*2,fs)

multiplos=np.zeros(0) #Se crea una rutina para hallar los multiplos por 500 de la longitud de la señal filtrada
for i in np.arange(len(Pulso_filtrado)):
    resto=i % 10
    if resto==0:
        multiplos=np.append(multiplos,i)
        
        valor=int(np.max(multiplos)) #Se halla el mallor multiplo de 500 de la señal filtrada         
        senal_filtrada_pasabajas=Pulso_filtrado[0:valor]#Se genera un vector de la señal filtrada hasta el valor del maximo multiplo


muestras=np.split(senal_filtrada_pasabajas,10,axis=0) #Se generan las épocas cada 50o muestras
promedio=np.mean(senal_filtrada_pasabajas)
senal_resultante=np.zeros(0) #Se genera un vector vacio
...
for i in muestras: #Se genera una rutina para extraer las épocas y concatenar las de interes y generar la señal filtrada definitiva
    ...
    #plt.show()
plt.plot(Pulso)
plt.title('Muetras puras tomadas del fotodiodo')
plt.grid()
plt.show()
plt.plot(f,pot)
plt.title('Periodograma Muestras sin filtrar')
plt.xlabel('Frecuencia')
plt.ylabel('Potencia')
plt.grid()
plt.show()

plt.plot(senal_resultante)
plt.title('Señal filtrada con pasa bajas de 7hz y pasa altas de 1hz')
plt.grid()
plt.show()
plt.plot(f2,pot2)
plt.title('Periodograma de la señal filtrada')
plt.xlabel('Frecuencia')
plt.ylabel('Potencia')
plt.grid()
plt.show()
...
Pulso=pd.read_csv('Datos_sin_filtros.csv')
Pulso=Pulso['Datos']
Pulso_filtrado=signal.filtfilt(lowpass, 1, Pulso)
f,pot= signal.welch(Pulso,fs, 'hanning', fs*2,fs)
f2,pot2=signal.welch(Pulso_filtrado,fs, 'hanning', fs*2,fs)
...
for i in muestras: #Se genera una rutina para extraer las épocas y concatenar las de interes y generar la señal filtrada definitiva
    ...
    #plt.show()
plt.plot(Pulso)
plt.title('Señal sin filtros pero con amplificación')
plt.grid()
plt.show()
plt.plot(f,pot)
plt.title('Periodograma de la señal sin filtrar')
plt.xlabel('Freciuencia')
plt.ylabel('Potencia')
plt.grid()
plt.show()

plt.plot(Pulso_filtrado)
plt.title('Señal filtrada con pasa bajas de 7hz y pasa altas de 1hz')
plt.grid()
plt.show()
plt.plot(f2,pot2)
plt.title('Potencia de la señal filtrada')
plt.xlabel('Frecuencia')
plt.ylabel('Potencia')
plt.grid()
plt.show()

Pulso=pd.read_csv('Datos_sin_filtro_analogico_con_movimiento.csv')
Pulso=Pulso['Datos']
Pulso_filtrado=signal.filtfilt(lowpass, 1, Pulso)
f,pot= signal.welch(Pulso,fs, 'hanning', fs*2,fs)
f2,pot2=signal.welch(Pulso_filtrado,fs, 'hanning', fs*2,fs)
...
muestras=np.split(senal_filtrada_pasabajas,50,axis=0) #Se generan las épocas cada 50o muestras
promedio=np.mean(senal_filtrada_pasabajas)
senal_resultante=np.zeros(0) #Se genera un vector vacio
...
for i in muestras: #Se genera una rutina para extraer las épocas y concatenar las de interes y generar la señal filtrada definitiva
    ...
    #plt.show()
plt.plot(Pulso)
plt.title('Señal sin filtros passa bajas y con artefactos de movimiento')
plt.grid()
plt.show()
plt.plot(f,pot)
plt.title('Periodograma de la señal sin filtrar')
...
plt.plot(senal_resultante)
plt.title('Señal filtrada con Pasa bajas de 7hz, pasa altas de 1hz')
plt.grid()
plt.show()
plt.plot(f2,pot2)
plt.title('Periodograma de la señal filtrada filtrada')
plt.xlabel('Frecuencia')
plt.ylabel('Potencia de la señal')
plt.grid()
plt.show()
q
datos=pd.DataFrame({'Señal':[]})
datos.to_csv('Senal_filtrada.csv')
datos.append({'Señal':[senal_resultante]}, ignore_index=True)

pb.uploadfile('Senal_filtrada.csv','Senal_filtrada.csv','text/csv')
